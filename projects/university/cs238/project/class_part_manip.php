...
{ 
  ...
  {  
    ...
    $zip_num = (int)$q[Zip];
    $bcbs_num = (int)$q[BCBSNum];
    $bcbs_suffix = (int)$q[BCBSSuffix];
    ...
    {    
      ...
      //Event_ID must be the same as a class in the event table on insertion
      else if (count( my_trusted_mysql_query("SELECT Event_ID FROM EVENT WHERE Event_ID = $event_id AND E_Type = 'c'")) == 0)
      {
        echo "Invalid Event ID entered. There is no class with that Event ID. <br />";
      }
      //Person is already in the CLASS_PARTICIPANT table.
      else if( count( my_trusted_mysql_query("SELECT Email FROM CLASS_PARTICIPANT WHERE Email = '$q[Email]'")) > 0) 
      {
        //see if person is already in C_ATTEND table
        if( count( my_trusted_mysql_query("SELECT Email FROM C_ATTEND WHERE Email = '$q[Email]' AND Event_ID = $event_id")) > 0)
        {
            echo "The Person with the e-mail " . $q['Email'] . " is already attending the class. " .
        ...
        {
            //Perform the insertion into C_ATTEND
            $query = "INSERT INTO C_ATTEND (Event_ID, Email)
            VALUES ('$q[Email]', $event_id)";

            my_trusted_mysql_query( $query );

            echo "Record inserted: <br />";
            display_results(my_trusted_mysql_query( "SELECT * FROM C_ATTEND WHERE Email = '$q[Email]' AND Event_ID = $event_id" ));
        }
      }
    
      else
      {
        //Perform the insertion into CLASS_PARTICIPANT
        $query = "INSERT INTO CLASS_PARTICIPANT (Email, Fname, Lname, Address, City, State, Zip, Comments, BCBSNum, BCBSSuffix, Birth_Date) 
        VALUES ('$q[Email]', '$q[Fname]', '$q[Lname]', '$q[Address]', '$q[City]', '$q[State]', $zip_num, '$q[Comments]', $bcbs_num, $bcbs_suffix, '$q[Birth_Date]')";

        my_trusted_mysql_query( $query );
        
        echo "Your information as a class particiant: <br />";
        display_results(my_trusted_mysql_query( "SELECT * FROM CLASS_PARTICIPANT WHERE Email = '$q[Email]'" ));

        //Perform the insertion into C_ATTEND
        $query = "INSERT INTO C_ATTEND (Event_ID, Email) VALUES ($event_id, '$q[Email]')";
    ...
    else if ($_POST['query_type'] == 'update')
    {
      ...
      if( count( my_trusted_mysql_query("SELECT Email FROM CLASS_PARTICIPANT WHERE Email = '$q[Email]'") ) == 0 )
      {
        echo "The person with the e-mail " . $q['Email'] . " is not currently in the database as class participant. " .
      ...
      {                
        ...
        $query  = "UPDATE CLASS_PARTICIPANT SET ";
        $query .=                       "Email='$q[Email]'";
        ...
        $query .= $q['BCBSNum']     != '' ? ", BCBSNum='$bcbs_num'"        : "";
        $query .= $q['BCBSSuffix']     != '' ? ", BCBSSuffix='$bcbs_suffix'"        : "";
        $query .= $q['Birth_Date']     != '' ? ", Birth_Date='$q[Birth_Date]'"        : "";
    ...
    {    
      ...
      if( count( my_trusted_mysql_query("SELECT Email FROM CLASS_PARTICIPANT WHERE Email = '$q[Email]'") ) == 0 )
      {
        echo "The person with the e-mail " . $q['Email'] . " is not currently in the database as a class participant. " .
      ...
      {        
        ...
        display_results( my_trusted_mysql_query( "SELECT * FROM CLASS_PARTICIPANT WHERE Email = '$q[Email]'" ));
        display_results( my_trusted_mysql_query( "SELECT * FROM C_ATTEND WHERE Email = '$q[Email]'" ));

        $query = "DELETE FROM CLASS_PARTICIPANT WHERE Email='$q[Email]'";
...
?>
          ...
          <div class="style35">
            <ul>
              <strong>Class participant insert/update/delete instructions:</strong><br />
              <li>The event ID of the class must be be given when inserting a new class particiant.</li>
              <li>When inserting a new class participant, you need to supply as much information as possible about the 
                  person.</li>
              <li>When deleting a class participant, you need only supply the person's e-mail address.<br /></li>
              <li>When updating a class participant's information, make sure the e-mails match, and leave any fields you do not want to change blank.<br /></li>
            </ul>
...
" />                </td>
            ...
            <tr>
            <td class="style36">BCBS Number:</td>
            <td class="style36">
            <input type="text" name="BCBSNum" value="
<?php 
              if (isset($_POST['submit']) && $_POST['Fill'] == 'true') 
              {
               echo $_POST['BCBSNum'];
...
?>" />                </td>
            ...
            <tr>
            <td class="style36">BCBS Suffix:</td>
            <td class="style36">
            <input type="text" name="BCBSSuffix" value="
<?php 
              if (isset($_POST['submit']) && $_POST['Fill'] == 'true') 
              {
               echo $_POST['BCBSSuffix'];
