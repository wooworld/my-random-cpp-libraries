...
/// @note Uses a lot of m_rows and m_cols instead of height() and width() for
///   slightly more speed.
...
template <typename T>
Vector<T> Matrix<T>::get_row( const unsigned long int& n ) const
{
  if ( n > m_rows )
...
template <typename T>
Vector<T> Matrix<T>::get_col( const unsigned long int& n ) const
{
  if ( n > m_cols )
