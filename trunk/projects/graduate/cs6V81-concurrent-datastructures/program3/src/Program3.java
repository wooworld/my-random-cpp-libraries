...
public class Program3 
{
	...
	public static void main( String[] args ) 
	{
		...
		int iterations = 960400, minBackoff = 1, maxBackoff = 20, exchangerCapacity = 2, exchangerWaitDuration = 10;
		int maxThreads = 16;
		System.out.println("LockBasedStack");
		for(int numThreads = 2; numThreads < maxThreads; numThreads++)
		{
				MyStack<Integer> theLock = new LockBasedStack<Integer>();
				testStack(theLock, numThreads, iterations);
		}
		System.out.println("LockFreeStack");
		for(int numThreads = 2; numThreads < maxThreads; numThreads++)
		{
				MyStack<Integer> theLock = new LockFreeStack<Integer>(minBackoff, maxBackoff);
				testStack(theLock, numThreads, iterations);
		}
	...
	public static void testStack(final MyStack<Integer> theStack, int numThreads, final int numOperations)
	{
		...
		totalTimeToUnLock = 0;
		  ...
		  for(int i=0; i<numThreads; i++)
		  {
			...
			System.out.println(theStack.getStackInfo() + "," + numThreads + "," + totalTimeToLock/numThreads + "," + totalTimeToUnLock/numThreads);
  }
}
