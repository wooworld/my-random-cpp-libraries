...
int node_set_id( char* n ) {
  NODE_ID = str_to_uint( string(n) );
  
  return 0;
}

int node_submit_results() {
  cout << "Preparing results for submission... " << NODE_NETWORK_MASTER << endl;
  ...
  log << "CS Entry Log for node " << NODE_ID << endl;
  ...
  string output_path = "results" + int_to_str( NODE_ID ) + ".txt";
...
int node_print_results() {
  if ( NODE_ID == NODE_NETWORK_MASTER ) {
