...
{ 
  ...
  else
  {
    ...
    display_results( my_trusted_mysql_query(    
      "SELECT * FROM STAFF WHERE Email IN (SELECT Email FROM ADMINISTER WHERE Event_ID=$Event_ID_num)" 
    ));
    display_results( my_trusted_mysql_query(    
      "SELECT * FROM STAFF WHERE Email IN (SELECT Email FROM TEACH WHERE Event_ID=$Event_ID_num)" 
    ));
...
<form action="special_event_participants.php" method="POST">
